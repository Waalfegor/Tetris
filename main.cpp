...
bool endGame(RenderWindow *window)
{
	for (int i = 0; i < N; i++)
	{
		if (field[2][i]) // замість 0нада 1 так правильніше
		{
			window->close();
			return false;
		}
...
int main()
{
	...
	texture.loadFromFile("C:\\dev\\tiles.png");
	texture_background.loadFromFile("C:\\dev\\background.png");
	texture_frame.loadFromFile("C:\\dev\\frame.png");
	texture_game_over.loadFromFile("C:\\dev\\game_over(1).png");
	...
	while (window.isOpen() && endGame(&window))
	{
	...
	while (window2.isOpen())
	{
		...
		while (window2.pollEvent(event))
		{
			...
			if (event.type == Event::Closed) window2.close();
		}
